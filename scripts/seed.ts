...
const samplePosts = [
	{
		...
		summary: "介紹如何透過 Next.js 16、Drizzle ORM 與 PostgreSQL 打造自動同步的技術部落格。",
		content: `# GitxBlog
...
使用 \\`ts_vector\\` 與 \\`websearch_to_tsquery\\` 實作全文搜尋。`,
...
async function main() {
	...
	const [author] = await db
		...
		.onConflictDoUpdate({ target: authors.slug, set: { displayName: "Gitx 管理員" } })
	...
	for (const post of samplePosts) {
		...
		const categoryRows = await Promise.all(post.categories.map((category) => upsertCategory(category)))

		if (tagRows.length > 0) {
			await db.insert(postTags).values(
				tagRows.map((tag) => ({ postId, tagId: tag.id })),
			)
		}
